...
export default function renderLayout(props) {
    const { content, documentPath, metadata, site, title } = props;
    const trackingUrl = getTrackingUrl(documentPath, title);
    return u('root', [
        u('doctype'),
        h('html', [
            h('head', [
                ...
                h('link', { rel: 'canonical', href: BASE_URL + documentPath }),
                metadata,
            ]),
            h('body', [
                h('div', { class: 'l-root' }, [
...
function getTrackingUrl(documentPath, title) {
    ...
    url.searchParams.set('dp', documentPath);
