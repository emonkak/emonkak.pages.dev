...
const CHUNK_SIZE = 16;
const NORMALIZE_URL_PATH_PATTERN = /index\.html$/;

export class ResourceLoader {
    static async init(options = {}) {
        const textmateRegistry = options.textmateRegistry ?? createTextmateRegistry();
        const graphviz = options.graphviz ?? (await Graphviz.load());
        const withoutCache = options.withoutCache ?? false;
        return new ResourceLoader(textmateRegistry, graphviz, withoutCache);
    }
    ...
    createHttpHandler(site) {
        const endPoints = site.allResources().reduce((acc, resource, index) => {
            acc[resource.url] = { resource, index };
            return acc;
        }, {});
    ...
    async _loadResource(srcPath, logicalPath) {
        ...
        case 'md': {
            ...
            return {
                ...
                srcPath,
                mountPath,
                url: '/' + mountPath.replace(NORMALIZE_URL_PATH_PATTERN, ''),
        ...
        case 'js': {
            ...
            if (['html', 'svg', 'xml'].includes(innerExtension)) {
                ...
                return {
                    ...
                    srcPath,
                    mountPath: initPath,
                    url: '/' + initPath.replace(NORMALIZE_URL_PATH_PATTERN, ''),
                    timestamp: stats.mtimeMs,
                    data: { render },
                };
            }
        ...
        return {
            ...
            url: '/' + logicalPath.replace(NORMALIZE_URL_PATH_PATTERN, ''),
            timestamp: stats.mtimeMs,
            data: {},
        };
    }
