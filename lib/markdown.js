        ...
        for (const node of nodes) {
            if (node.properties.src) {
                ...
                if (imageUrl.protocol === 'file:') {
                    const metadata = await sharp(imageUrl.pathname).metadata();
                    const density = metadata.density > 0 ?
                        Math.ceil(metadata.density / 72) :
                        1;
                    Object.assign(node.properties, {
                        width: metadata.width / density,
                        height: metadata.height / density,
                    });
                }
            }
        }
...
function remarkExtractSections() {
    return (tree, file) => {
        const root = { children: [], index: 0, depth: 0 };
        const sectionStack = [];

        visit(tree, 'heading', (node) => {
            let parentSection = root;

            while (sectionStack.length > 0) {
                const section = sectionStack.at(-1);
            ...
            const index = parentSection.children.length;
            const id = 'section-' + [
                ...sectionStack.map((section) => section.index + 1),
                index + 1,
            ].join('-');
            const section = {
        ...
        visit(tree, test, (node, index, parent) => {
            ...
            if (node.type === 'code') {
                ...
                if (node.title || node.alt) {
                    captionChildren.push({ type: 'text', value: node.title ?? node.alt });
                }
