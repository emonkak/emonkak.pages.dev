...
function rehypeFlattenFooter() {
    return (tree, _file) => {
        ...
        visit(tree, test, (node, index, parent) => {
            ...
            const footnoteList = node.children[2];
            parent.children.splice(index, 1, footnoteLabel, footnoteList);
        });
    };
}
...
function rehypeGraphviz(options) {
    ...
    return (tree, _file) => {
        ...
        visit(tree, test, (node, index, parent) => {
            ...
            const svg = cleanSvg(output);

            unsetDefaultFontFamily(svg);
...
function unsetDefaultFontFamily(tree) {
    ...
    return async (tree, file) => {
        ...
        for (const { node, language } of nodes) {
            ...
            const codeElement = {
                type: 'element',
                tagName: 'code',
                children: tokensByLine.map((tokens) => ({
                    type: 'element',
                    tagName: 'div',
                    children: tokens.length > 0 ? tokens.map(createTokenNode) : [{ type: 'text', value: '\n' }],
                })),
            };
...
function createTokenNode(token) {
    ...
    if (token.explanations) {
        node.properties['data-explanations'] = JSON.stringify(token.explanations);
    }
