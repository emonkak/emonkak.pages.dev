...
export function createHighlightTree(text, grammar) {
    ...
    const root = {
        ...
        tagName: 'code',
        properties: {
            className: scopeToClassName(rootScope),
        },
        children: [],
    };
    const scopeStack = [];
    ...
    let start = 0;
    let stateStack = textmate.INITIAL;
...
function unstableGetScopeNameFromGrammar(grammar) {
    const scopeName = grammar._rootScopeName;
    assert.equal(typeof scopeName, 'string');
    return scopeName;
}
